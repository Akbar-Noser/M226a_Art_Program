...
public class Painter {
    ...
    public void createColorTemplate(List<Color> colors) throws IOException {
        BufferedImage bi = new BufferedImage(Canvas.getCanvasX(), Canvas.getCanvasY(), BufferedImage.TYPE_INT_RGB);
        for (int i = 1; i <= colors.size(); i++) {
            for (int k = 0; k < Canvas.getCanvasX() / colors.size(); k++) {
                for (int j = 0; j < Canvas.getCanvasY(); j++) {
                    bi.setRGB(k + Canvas.getCanvasX() / colors.size() * (i - 1), j, colors.get(i - 1).getRGB());
                }
            }
        }
    ...
    public void createStripes(boolean evenSpaces, int weight, List<Color> colors) throws IOException {
        ...
        BufferedImage bi = new BufferedImage(Canvas.getCanvasX(), Canvas.getCanvasY(), BufferedImage.TYPE_INT_RGB);
        if (evenSpaces) {
            for (int k = 0; k < Canvas.getCanvasX(); k++) {
